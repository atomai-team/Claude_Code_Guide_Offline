...
## 🌟 高级功能在 PM 工作流中的使用（2026-06-25 增）

> PM 视角下 5 大高级功能怎么用——管理视角，与工程视角不同。

| 场景 | 推荐入口 | 做什么 |
|---|---|---|
| 周一发现项目混乱 | `秘书` daily-brief + weekly-secretary | 看自动健康报告，先量化再行动 |
| 不确定哪个 skill 能用 | `super` 不知道找谁 | 主调度器列候选让你选（路由层兜底） |
| 跨部门决策（市场+技术+财务） | `CEO` ceo-mode 拉 7 顾问 | 一次跑完，避免来回开多个 session |
| 多个并行项目需要全权代理 | `队长` smart-flow-all | 极限档派 subagent 并行处理 |
| 多 session 协调 | `经理` multi-session-state（自动） | 看谁在改什么文件，冲突检测 |

> 💡 **PM 实战建议**：周一早上看 `秘书` 健康报告 → 不确定的事用 `super` 兜底 → 重要决策用 `CEO` → 极限任务用 `队长`。不要一开始就上 `队长`（容易黑盒改坏）。

### 📋 PM 视角的 5 大高级功能进阶（2026-06-25 增）

> 给 PM / 技术负责人的精细化使用 · 与工程师视角的"全权极限档"不同。

**1. 周一晨会 30 分钟自动健康巡检**

```bash
# 触发"秘书"自动跑周报 + 健康检查
"秘书，跑下 weekly-driver 和 memory-health"
# → 输出：上周改了什么 · 配置漂移数 · 记忆碎片 · 下周该清理什么
```
...
| 部门 | 推荐入口 | 配合动作 |
|---|---|---|
| 产品+技术+市场分歧 | `CEO 启动 评估 X 方案` | 7 顾问并列 45 分钟出报告 |
| 不确定该用哪个 skill | `super 我想做 X` | 路由器列候选 |
| 团队产出质量存疑 | `队长 + /code-review ultra` | 独立 reviewer + 异构对抗 |

**3. 项目分阶段升级策略**

- **Phase 1 启动**：`super 启动项目 X`（轻量）
- **Phase 2 调研**：`CEO + 调研 X 赛道`（商业可行性）
- **Phase 3 工程**：`队长 工程化 MVP`（极限档全权）
- **Phase 4 维护**：`秘书 weekly-driver`（自动化巡检）

> 💡 **PM 警示**：Phase 4 后**不要默认开队长**，容易改坏稳态；用 `秘书` + `/code-review` 替代。

**4. 多 session 协调的 PM 实务**

`经理` 不是手动触发的，是 **自动激活**。PM 实务里：
- 同时跑 3 个项目（每个项目一个 session）→ `经理` 自动显示活跃列表
- 切换项目前看 `经理` 列出的 "谁在改什么文件"
- 冲突时（同一文件多人改）→ `经理` 红字警告

> 📖 **详见** dashboard.html §C10.5-10.10 + mycc-config.html §"高级功能" + start-here.md。
