...
class TestSectionRhythmContract:
    ...
    def test_rhythm_count_is_12(self, rhythm_color_sequence):
        """实测 16 section-rhythm: 15 + s-orchestration(purple) = 16. 全 19 section 中 3 个无节奏."""
        assert len(rhythm_color_sequence) == 16, (
            f"section-rhythm 数量 {len(rhythm_color_sequence)} ≠ 16. "
            f"实测: {rhythm_color_sequence}"
        )

    def test_color_sequence_cycles_6(self, rhythm_color_sequence):
        """6 色循环 2 轮 (12) + s-resources(cyan) + s-health(purple) + s-architecture(blue) + s-advanced(cyan) + s-orchestration(purple) + s-scenarios(orange) = 16."""
        expected = ["cyan", "orange", "accent", "green", "purple", "blue",
                    "cyan", "orange", "accent", "cyan", "green", "purple", "blue", "cyan", "purple", "orange"]
